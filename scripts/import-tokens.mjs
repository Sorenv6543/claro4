...
function collectLeaves(obj, pathArr, collection, out) {
  for (const [key, val] of Object.entries(obj)) {
    ...
    if (val && '$value' in val) {
      ...
      if (entry) out.push({ entry, newValue: val.$value })
...
for (const { entry, newValue } of updates) {
  ...
  const escapedVar = entry.css.replace(/[-]/g, '\\-')
  const re = new RegExp('(' + escapedVar + ':\\s+)' + '[^;/\\n]+?' + '(;|\\s*\\/\\*)')
...
function formatForCss(value, type, stripUnit) {
  if (type === 'color') return String(value).toUpperCase()
  if (type === 'number') {
