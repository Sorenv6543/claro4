...
async function analyzeBuildSize () {
  ...
  try {
    ...
    console.log('===================================')
    for (const [file, size] of roleChunks) {
      const sizeKB = (size / 1024).toFixed(2)
...
function getChunkType (filename) {
  ...
  if (lowerFile.includes('admin') && lowerFile.includes('component')) {
    return 'admin-components'
  }
  if (lowerFile.includes('owner') && lowerFile.includes('component')) {
    return 'owner-components'
  }
  if (lowerFile.includes('shared') && lowerFile.includes('ui')) {
    return 'shared-ui'
  }
  if (lowerFile.includes('admin') && lowerFile.includes('logic')) {
    return 'admin-logic'
  }
  if (lowerFile.includes('owner') && lowerFile.includes('logic')) {
    return 'owner-logic'
  }
  if (lowerFile.includes('shared') && lowerFile.includes('logic')) {
    return 'shared-logic'
  }
  if (lowerFile.includes('vuetify')) {
    return 'vuetify'
  }
  if (lowerFile.includes('vue-core') || (lowerFile.includes('vue') && !lowerFile.includes('vuetify'))) {
    return 'vue-core'
  }
  if (lowerFile.includes('calendar')) {
    return 'calendar'
  }
...
function getChunkStatus (sizeKB, threshold) {
  if (sizeKB <= threshold.target) {
    return '🎯'
  } // At target
  if (sizeKB <= threshold.target * 1.1) {
    return '✅'
  } // Within 10% of target
  if (sizeKB <= threshold.max * 0.8) {
    return '🟢'
  } // Good
...
async function trackPerformanceRegression (currentSnapshot) {
  try {
    ...
    if (history.length >= 2) {
      ...
      const alerts = []
      if (sizeDiff > 100) {
        alerts.push('⚠️ Bundle size increased by >100KB')
      }
      if (current.totalSize > PERFORMANCE_THRESHOLDS.maxTotalBundleSize) {
        alerts.push('🔴 Bundle size exceeds threshold')
      }
      if (current.chunkCount > PERFORMANCE_THRESHOLDS.maxChunkCount) {
        alerts.push('🟡 Too many chunks generated')
      }
...
async function _analyzeServiceWorker () {
  try {
...
async function analyzeImportPaths () {
  ...
  if (importAnalysis.issues.length > 0) {
    ...
    for (const issue of importAnalysis.issues.slice(0, 10)) {
