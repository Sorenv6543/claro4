...
function ownerEmail (name) {
  ...
  const lastInitial = parts.length > 1 ? parts.at(-1)[0].toLowerCase() : ''
...
function fixEncoding (str) {
  let s = str
  for (const [pat, rep] of ENCODING_FIXES) {
    s = s.replace(pat, rep)
  }
  return s
}
...
function addressFingerprint (raw) {
  ...
  if (streetTokens.length > 1 && DIRECTIONALS.has(streetTokens[0])) {
    streetTokens.shift()
  }
...
function pickCanonical (addresses) {
  return [...addresses].toSorted((a, b) => b.length - a.length)[0]
}
...
function parseSummary (summary) {
  ...
  if (prefix === 'TURN' || prefix?.startsWith('T/U')) {
    booking_type = 'turn'
  }
...
for (const b of bookings) {
  ...
  if (!canonicalMap.has(fp)) {
    canonicalMap.set(fp, new Set())
  }
...
for (const b of bookings) {
  if (fpRemap.has(b._address_key)) {
    b._address_key = fpRemap.get(b._address_key)
  }
}
...
const sortedByAddress = Object.fromEntries(
  Object.entries(byAddress).toSorted(([, a], [, b]) =>
...
for (const { canonical_address, _variants, bookings: bks, owner_name, matched } of Object.values(sortedByAddress)) {
  const _dates = bks.map(b => (b.checkout_date ?? b.checkin_date).slice(5)).join(', ')
