            ...
            <div style={{ paddingTop: 10 }}>
              ...
              >
                {featured.today.length > 0 && (
                  <>
                    {featured.today[0].time}
                    {' '}
                    ·
                    {typeLabel(featured.today[0].type)}
                    {' '}
                    ·
                    {' '}
                    {featured.today[0].guests}
                    {' '}
                    guests expected.
                  </>
                )}
                ...
                {featured.today.map(ev => (
                  <div key={ev.id} style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                    <div style={{ fontSize: 14, fontWeight: 800, color: INK_B1ED, fontVariantNumeric: 'tabular-nums',
                      letterSpacing: '-0.01em', minWidth: 42 }}
                    >
                      {ev.time}
                    </div>
                    <div style={{ flex: 1 }}>
          ...
          {NEEDSACTION.length > 0
            ? (
                <>
                  ...
                  ·
                  {' '}
                  {heroes.length - NEEDSACTION.length}
                  {' '}
                  active ·
                  {' '}
                  {quiet.length}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {p.today.map(ev => (
                    <div key={ev.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <span style={{ fontSize: 12, color: OV.fg2, flex: 1 }}>
                        {typeLabel(ev.type)}
                        {' '}
                        ·
                        {' '}
