import type { ConfirmDialogState, FilterState, FilterValue, ModalData, ModalState, Notification, NotificationType } from '@/types'
...
export const useUIStore = defineStore('ui', () => {
  ...
  const anyLoading = computed((): boolean => {
    return Array.from(loading.value.values()).includes(true)
  })
  ...
  function closeAllModals () {
    for (const [id, modal] of modals.value.entries()) {
      modals.value.set(id, {
        ...modal,
        open: false,
      })
  ...
  function closeAllConfirmDialogs () {
    for (const [id, dialog] of confirmDialogs.value.entries()) {
  ...
  function addNotification (type: NotificationType, title: string, message: string, autoClose = true) {
