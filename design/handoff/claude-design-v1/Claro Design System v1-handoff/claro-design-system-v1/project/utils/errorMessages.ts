...
import type { BusinessImpact, ErrorCategory, UserRole } from '@/types/ui'
...
export function getErrorMessage (
  ...
  for (const [key, value] of Object.entries(context)) {
    message = message.replace(new RegExp(String.raw`\{${key}\}`, 'g'), String(value))
  }
...
export function assessBusinessImpact (
  ...
  if (category === 'system' && context.systemWide) {
    return 'critical'
  }
  if (context.dataLoss || context.securityBreach) {
    return 'critical'
  }
  ...
  if (affectedUsers > 10 || affectedProperties > 5) {
    return 'high'
  }
  if (category === 'authentication' && context.multipleUsers) {
    return 'high'
  }
  ...
  if (affectedBookings > 3 || revenueImpact > 500) {
    return 'medium'
  }
