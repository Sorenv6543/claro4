...
export function useAdminErrorHandler () {
  ...
  function assessPropertyBusinessImpact (context: {
    ...
    if (propertyCount > 10 || bookingCount > 20 || cleanerCount > 5) {
      return 'critical'
    }
    if (propertyCount > 5 || bookingCount > 10 || cleanerCount > 2) {
      return 'high'
    }
    if (propertyCount > 1 || bookingCount > 3) {
      return 'medium'
    }
    return 'low'
  }
  ...
  function assessBookingBusinessImpact (context: {
    ...
    if (revenue > 5000 || clients > 10) {
      return 'critical'
    }
    if (revenue > 1000 || clients > 5) {
      return 'high'
    }
    if (revenue > 200 || clients > 1) {
      return 'medium'
    }
    return 'low'
  }
  ...
  function assessCleanerBusinessImpact (context: {
    ...
    if (cleaners > 5 || bookings > 15 || conflicts > 10) {
      return 'critical'
    }
    if (cleaners > 2 || bookings > 8 || conflicts > 5) {
      return 'high'
    }
    if (cleaners > 1 || bookings > 3 || conflicts > 2) {
      return 'medium'
    }
    return 'low'
  }
  ...
  function assessPerformanceBusinessImpact (context: {
    ...
    const responseRatio = context.responseTime && context.threshold
      ? context.responseTime / context.threshold
      : 1
    const users = context.affectedUsers || 0
    const load = context.systemLoad || 0

    if (responseRatio > 5 || users > 50 || load > 90) {
      return 'critical'
    }
    if (responseRatio > 3 || users > 20 || load > 75) {
      return 'high'
    }
    if (responseRatio > 2 || users > 5 || load > 60) {
      return 'medium'
    }
    return 'low'
  }
