...
export function useCalendarState () {
  ...
  function updateDateRange () {
    let date: Date
    if (currentDate.value instanceof Date && !Number.isNaN(currentDate.value.getTime())) {
  ...
  function toggleStatusFilter (status: 'pending' | 'scheduled' | 'in_progress' | 'completed' | 'cancelled') {
    ...
    switch (status) {
      case 'pending': {
        filterValue = showPendingBookings.value
        break
      }
      case 'scheduled': {
        filterValue = showScheduledBookings.value
        break
      }
      case 'in_progress': {
        filterValue = showInProgressBookings.value
        break
      }
      case 'completed': {
        filterValue = showCompletedBookings.value
        break
      }
      default: { filterValue = showCancelledBookings.value
      }
    }

    uiStore.setFilter(`show${status.charAt(0).toUpperCase() + status.slice(1)}`, filterValue)
  }
