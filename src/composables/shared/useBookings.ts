...
export function useBookings () {
  ...
  async function createBooking (formData: BookingFormData): Promise<string | null> {
    ...
    try {
      ...
      if (Number.isNaN(checkoutDate.getTime()) || Number.isNaN(checkinDate.getTime())) {
        throw new TypeError('Invalid dates provided')
      }
    ...
    try {
      ...
      if (updates.checkout_date) {
        ...
        if (Number.isNaN(checkoutDate.getTime())) {
          throw new TypeError('Invalid checkout date')
        }
      ...
      if (updates.checkin_date) {
        ...
        if (Number.isNaN(checkinDate.getTime())) {
          throw new TypeError('Invalid checkin date')
        }
        shouldRecalculateType = true
      }
  ...
  async function deleteBooking (id: string): Promise<boolean> {
    ...
    try {
      ...
      error.value = error_ instanceof Error ? error_.message : 'Failed to delete booking'
      loading.value = false
      return false
    }
  }
  ...
  async function changeBookingStatus (id: string, status: BookingStatus): Promise<boolean> {
    ...
    try {
      ...
      error.value = error_ instanceof Error ? error_.message : 'Failed to update booking status'
      loading.value = false
      return false
    }
  }
