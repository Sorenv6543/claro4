...
describe('useSupabaseAuth', () => {
  ...
  describe('getAllUsers()', () => {
    it('fetches all users ordered by created_at descending', async () => {
      const users = [
        makeUserProfile({ id: 'user-1' }),
        makeUserProfile({ id: 'user-2' }),
      ]
    ...
    it('throws on query error', async () => {
      ...
      await expect(auth.getAllUsers()).rejects.toThrow('Query failed')
    })
  })
  ...
  describe('updateUserRole()', () => {
    it('updates user role successfully', async () => {
      ...
      const result = await auth.updateUserRole('user-123', 'admin')
    ...
    it('handles role update error', async () => {
      ...
      vi.mocked(supabaseMock.from).mockReturnValueOnce({
        update: vi.fn().mockReturnValueOnce({
          eq: vi.fn().mockResolvedValueOnce({ data: null, error }),
        }),
      } as any)
  ...
  describe('deleteUser()', () => {
    it('deletes user profile and auth user successfully', async () => {
      const profileDeleteMock = vi.fn().mockResolvedValueOnce({
        data: null,
        error: null,
      })
      const authDeleteMock = vi.fn().mockResolvedValueOnce({ error: null })

      vi.mocked(supabaseMock.from).mockReturnValueOnce({
        delete: vi.fn().mockReturnValueOnce({
          eq: profileDeleteMock,
        }),
      } as any)

      vi.mocked(supabaseMock.auth.admin.deleteUser).mockImplementationOnce(
        authDeleteMock,
      )

      const auth = await getComposable()

      const result = await auth.deleteUser('user-123')
    ...
    it('returns false if profile deleted but auth delete fails', async () => {
      ...
      vi.mocked(supabaseMock.auth.admin.deleteUser).mockResolvedValueOnce({
        error: authError,
      })
      ...
      expect(result).toBe(false)
      expect(auth.error.value).toContain('Profile deleted but auth record cleanup failed')
    })

    it('handles profile delete error', async () => {
      ...
      expect(result).toBe(false)
      expect(auth.error.value).toBe('Profile delete failed')
    })
  })

  describe('createAdminUser()', () => {
    it('creates admin user successfully', async () => {
      const authUserId = 'new-admin-id'
      const authCreateMock = vi.fn().mockResolvedValueOnce({
        data: { user: { id: authUserId, email: 'admin@example.com' } },
        error: null,
      })

      const profileInsertMock = vi.fn().mockResolvedValueOnce({
        data: null,
        error: null,
      })

      vi.mocked(supabaseMock.auth.admin.createUser).mockImplementationOnce(
        authCreateMock,
      )
      ...
      expect(result).toBe(true)
      expect(authCreateMock).toHaveBeenCalledWith(
        expect.objectContaining({
          email: 'admin@example.com',
          password: 'password123',
    ...
    it('defaults access_level to full', async () => {
      const authCreateMock = vi.fn().mockResolvedValueOnce({
    ...
    it('handles auth user creation error', async () => {
      const error = new Error('Auth user creation failed')
      vi.mocked(supabaseMock.auth.admin.createUser).mockResolvedValueOnce({
        data: { user: null },
        error,
      })
      ...
      expect(result).toBe(false)
      expect(auth.error.value).toBe('Auth user creation failed')
    })
    ...
    it('rolls back auth user if profile creation fails', async () => {
      ...
      expect(result).toBe(false)
      expect(authDeleteMock).toHaveBeenCalledWith(authUserId)
      expect(auth.error.value).toBe('Profile insert failed')
    })
  })
