...
<script setup lang="ts">
  import type { BusinessImpact, UserRole } from '@/types/ui'
  ...
  async function triggerValidationError (): Promise<void> {
    ...
    await (selectedRole.value === 'owner'
      ? ownerErrorHandler.handleFormError([error], 'property')
      : adminErrorHandler.handleSystemPropertyError(error, 'create', {
  ...
  async function triggerNetworkError (): Promise<void> {
    ...
    await (selectedRole.value === 'owner'
      ? ownerErrorHandler.handleNetworkError(error, 'save_property')
      : adminErrorHandler.handleIntegrationError(error, 'PropertyAPI', 'create', {
  ...
  async function triggerBusinessError (): Promise<void> {
    ...
    await (selectedRole.value === 'owner'
      ? ownerErrorHandler.handleBookingError(error, 'create')
      : adminErrorHandler.handleBookingManagementError(error, 'create', {
