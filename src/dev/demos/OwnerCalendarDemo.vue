...
<script setup lang="ts">
  ...
  function generateOwnerBookings () {
  ...
  const turnBookingsCount = computed(() => {
    return Array.from(ownerBookings.value.values())
      .filter(b => b.booking_type === 'turn')
      .length
  })

  const standardBookingsCount = computed(() => {
    return Array.from(ownerBookings.value.values())
      .filter(b => b.booking_type === 'standard')
      .length
  })
  ...
  function handleCreateBooking (data: { start: string, end: string, propertyId?: string }) {
  ...
  // Event logging
  function logEvent (from: string, to: string, event: string, type: 'emit' | 'receive', data?: any) {
