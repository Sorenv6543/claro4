...
<script setup lang="ts">
  ...
  function handleUpdateBooking (data: { id: string, updates: Partial<Booking> }): void {
  ...
  function handleAssignCleaner (data: { bookingId: string, cleanerId: string, notes?: string }): void {
  ...
  function handleUpdateBookingStatus (data: { bookingId: string, status: Booking['status'] }): void {
    const booking = demoBookings.value.get(data.bookingId)
    if (booking) {
  ...
  // Demo actions
  function addRandomBooking (): void {
